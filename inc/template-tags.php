...
if ( ! function_exists( 'sakufu_paging_nav' ) ) :
/**
 * Display navigation to next/previous set of posts when applicable.
 *
 * @since Sakufu 1.0
 */
function sakufu_paging_nav() {
...
if ( ! function_exists( 'sakufu_post_nav' ) ) :
/**
 * Display navigation to next/previous post when applicable.
 *
 * @since Sakufu 1.0
 */
function sakufu_post_nav() {
...
if ( ! function_exists( 'sakufu_posted_on' ) ) :
/**
 * Print HTML with meta information for the current post-date/time and author.
 *
 * @since Sakufu 1.0
 */
function sakufu_posted_on() {
	if ( is_sticky() && is_home() && ! is_paged() ) {
		echo '' . __( 'Sticky', 'sakufu' ) . '';
	}
 ...
 */
function sakufu_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'sakufu_category_count' ) ) ) {
		...
		set_transient( 'sakufu_category_count', $all_the_cool_cats );
	}

	if ( 1 !== (int) $all_the_cool_cats ) {
		// This blog has more than 1 category so sakufu_categorized_blog should return true
		return true;
	} else {
		// This blog has only 1 category so sakufu_categorized_blog should return false
		return false;
	}
}

/**
 * Flush out the transients used in sakufu_categorized_blog.
 *
 * @since Sakufu 1.0
 */
function sakufu_category_transient_flusher() {
	// Like, beat it. Dig?
	delete_transient( 'sakufu_category_count' );
}
add_action( 'edit_category', 'sakufu_category_transient_flusher' );
add_action( 'save_post',     'sakufu_category_transient_flusher' );
 ...
 * @since Sakufu 1.0
 */
function sakufu_post_thumbnail() {
	...
	<?php
		if ( ( ! is_active_sidebar( 'sidebar-2' ) || is_page_template( 'page-templates/full-width.php' ) ) ) {
			the_post_thumbnail( 'sakufu-full-width' );
		} else {
			the_post_thumbnail();
		}
	?>
